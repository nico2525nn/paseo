...
const convertZodToJsonSchema: (schema: z.ZodTypeAny, options: { $refStrategy: "none" }) => unknown =
  zodToJsonSchema as unknown as (
    schema: z.ZodTypeAny,
    options: { $refStrategy: "none" },
  ) => unknown;

export interface PaseoToolingRegistryOptions {
  ...
  /**
   * ID of the agent that is using Paseo tooling through an agent-scoped adapter.
   * Used for cwd/mode inheritance when agents spawn child agents.
   */
  callerAgentId?: string;
  agentScopedTools?: boolean;
  /**
...
export type PaseoToolHandler = (
  args: unknown,
  extra: McpToolContext,
) => Promise<CallToolResult> | CallToolResult;

type RegisterPaseoTool = <TInputSchema extends z.ZodRawShape>(
  name: string,
  config: PaseoToolConfig & { inputSchema?: TInputSchema },
  handler: (
...
export interface PaseoToolDefinition {
  name: string;
  config: PaseoToolConfig;
  handler: PaseoToolHandler;
}
...
export interface PaseoToolRegistry {
  listTools(): PaseoToolDefinition[];
  listManifest(): PaseoToolManifestEntry[];
  getTool(name: string): PaseoToolDefinition | null;
  executeTool(params: {
    name: string;
    input: unknown;
    callerAgentId?: string;
...
function buildRegistryApi(
  tools: Map<string, PaseoToolDefinition>,
  callerAgentIdOverride: AsyncLocalStorage<string | undefined>,
): PaseoToolRegistry {
  return {
    ...
    listManifest: () =>
      Array.from(tools.values()).map((tool) => ({
        name: tool.name,
        description: tool.config.description ?? tool.config.title ?? tool.name,
        inputJsonSchema: convertZodToJsonSchema(buildToolInputSchema(tool.config), {
          $refStrategy: "none",
        }),
      })),
    getTool: (name) => tools.get(name) ?? null,
    executeTool: async ({ name, input, callerAgentId }) => {
      const tool = tools.get(name);
      if (!tool) {
        throw new Error(`Unknown Paseo tool '${name}'`);
      }
      const parsedInput = await buildToolInputSchema(tool.config).parseAsync(input ?? {});
      return callerAgentIdOverride.run(callerAgentId, async () =>
        tool.handler(parsedInput as never, {} as never),
      );
    },
  };
}

function buildToolInputSchema(config: PaseoToolConfig): z.ZodTypeAny {
  return z.object(config.inputSchema ?? {}) as z.ZodTypeAny;
}
...
export function createPaseoToolRegistry(options: PaseoToolingRegistryOptions): PaseoToolRegistry {
  const {
    agentManager,
    agentStorage,
    terminalManager,
  ...
  const childLogger = logger.child({ module: "agent", component: "paseo-tooling" });
  const waitTracker = new WaitForAgentTracker(logger);
  const callerAgentIdOverride = new AsyncLocalStorage<string | undefined>();
  const tools = new Map<string, PaseoToolDefinition>();
  const registerTool: RegisterPaseoTool = function registerTool<TInputSchema extends z.ZodRawShape>(
    name: string,
    config: PaseoToolConfig & { inputSchema?: TInputSchema },
    handler: (
      args: z.infer<z.ZodObject<TInputSchema>>,
    ...
    const normalizedConfig = relaxMcpToolOutputSchema(config) as PaseoToolConfig;
    const wrappedHandler = (async (args: unknown, extra: McpToolContext) => {
      return addModelVisibleStructuredContent(
        await handler(args as z.infer<z.ZodObject<TInputSchema>>, extra),
      );
    }) as PaseoToolHandler;
    tools.set(name, {
      name,
  ...
  const resolveEffectiveCallerAgentId = () => callerAgentIdOverride.getStore() ?? callerAgentId;

  const resolveEffectiveCallerContext = () => {
    const effectiveCallerAgentId = resolveEffectiveCallerAgentId();
    return effectiveCallerAgentId ? (resolveCallerContext?.(effectiveCallerAgentId) ?? null) : null;
  };

  const resolveCallerAgent = () => {
    const effectiveCallerAgentId = resolveEffectiveCallerAgentId();
    if (!effectiveCallerAgentId) {
      return null;
    }
    const parentAgent = agentManager.getAgent(effectiveCallerAgentId);
    if (!parentAgent) {
      throw new Error(`Parent agent ${effectiveCallerAgentId} not found`);
    }
    return parentAgent;
  };
  ...
  const resolveScopedCwd = (requestedCwd?: string, opts?: { required?: boolean }): string => {
    ...
    if (callerAgent) {
      const callerContext = resolveEffectiveCallerContext();
  ...
  const createAgentInputSchema =
    options.agentScopedTools || callerAgentId ? agentToAgentInputSchema : topLevelInputSchema;
  ...
  if (
    ...
    resolveEffectiveCallerContext()?.enableVoiceTools
  ) {
    registerTool(
      "speak",
      {
      ...
      async (args, context?: McpToolContext) => {
        const effectiveCallerAgentId = resolveEffectiveCallerAgentId();
        if (!effectiveCallerAgentId) {
          throw new Error("speak is only available to agent-scoped Paseo tooling sessions");
        }
        const handler = resolveSpeakHandler?.(effectiveCallerAgentId) ?? null;
        if (!handler) {
          throw new Error(
            `No speak handler registered for your session '${effectiveCallerAgentId}'`,
          );
        }
        await handler({
          text: args.text,
          callerAgentId: effectiveCallerAgentId,
  ...
  if (options.voiceOnly) {
    return buildRegistryApi(tools, callerAgentIdOverride);
  }
    ...
    async (args: unknown) => {
      ...
      const {
        ...
        },
        {
          ...
          notifyOnFinish,
          detached,
          callerAgentId: resolveEffectiveCallerAgentId(),
          callerContext: resolveEffectiveCallerContext(),
          worktree,
        },
      );
      ...
      const effectiveCallerAgentId = resolveEffectiveCallerAgentId();
      const guidance =
        effectiveCallerAgentId && notifyOnFinish && initialPromptStarted
  ...
  function resolveCreateAgentToolArgs(args: unknown): ResolvedCreateAgentToolArgs {
    if (resolveEffectiveCallerAgentId()) {
      return {
        kind: "agent-scoped",
  ...
  registerTool(
    ...
    async ({ agentId, prompt, sessionMode, background = false, notifyOnFinish = false }) => {
      ...
      const effectiveCallerAgentId = resolveEffectiveCallerAgentId();
      if (notifyOnFinish && effectiveCallerAgentId) {
  ...
  registerTool(
    ...
    async ({ includeArchived = false, cwd, sinceHours = 48, statuses, limit = 50 }) => {
      const callerCwd = resolveEffectiveCallerAgentId() ? resolveCallerAgent()?.cwd : undefined;
  ...
  registerTool(
    ...
    async ({ prompt, cron, timezone, name, maxRuns, expiresIn }) => {
      ...
      const schedule = await scheduleService.create({
        ...
        target: { type: "agent", agentId: effectiveCallerAgentId },
        ...(name?.trim() ? { name: name.trim() } : {}),
        ...(maxRuns === undefined ? {} : { maxRuns }),
        ...(expiresAt === undefined ? {} : { expiresAt }),
      });
